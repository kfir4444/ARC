...
def main():
    ...
    try:
        ...
        save_output_file(path = str(args.yml_path), content = output)
        return
    ...
    if job_type in ['opt', 'conformers', 'optfreq']:
    ...
    if job_type == 'scan':
        NotImplementedError("Scan job type is not implemented for TorchANI. Use ARC's directed scan instead")
    return None
