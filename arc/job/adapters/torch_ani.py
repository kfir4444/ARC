...
TANI_SCRIPT_PATH = os.path.join(ARC_PATH, 'arc', 'job', 'adapters', 'scripts', 'tani_script.py')
...
class TorchANIAdapter(JobAdapter):
    ...
    def check_settings(self, settings):
        ...
        for key in options.keys():
            if not settings[key] in options[key]:
                return False
        if not isinstance(settings["fmax"], (float, int)):
            return False
        if not (isinstance(settings["steps"], int) or settings["steps"] is None):
            return False
        return True
    ...
    def execute_incore(self):
        ...
        if self.level is not None:
            if self.level.args is not None:
                if any(key in self.level.args for key in ["model", "device", "fmax", "engine", "steps"]):
                    temp_dict = {"model" : self.level.args["model"],
                                 "device" : self.level.args["device"],
                                 "fmax" : self.level.args["fmax"],
                                 "engine" : self.level.args["engine"],
                                 "steps": self.level.args["steps"],}
                    tani_default_options_dict.update(temp_dict)
