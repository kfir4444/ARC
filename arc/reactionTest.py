...
class TestARCReaction(unittest.TestCase):
    ...
    def test_get_atom_map(self):
        ...
        self.assertIn(rxn.atom_map[3:5], [[7, 8], [8, 7]])
        self.assertIn(tuple(rxn.atom_map[5:8]), list(permutations([4, 5, 9], 3)))
        self.assertIn(rxn.atom_map[8:], [[2, 3], [3, 2]])
        self.assertTrue(check_atom_map(rxn))
        ...
        atom_map = rxn.atom_map
        self.assertIn(atom_map[:2],[[0,1],[1,0]])
        self.assertEqual(atom_map[2], 2)
        self.assertIn(atom_map[3:5],[[7,8],[8,7]])
        for index in atom_map[5:8]:
            self.assertIn(index,[3,4,5])
        self.assertEqual(atom_map[8:],[6,9])
...
H       1.12853146   -0.86793870    0.06973060"""
    ...
    def test_get_atom_map_wip(self):
        ...
        # 1+2_Cycloaddition: CH2 + C2H4 <=> C3H6, waiting for issue #506
        c2h4_xyz = {'coords': ((0.6664040429179742, 0.044298334171779405, -0.0050238049104911735),
                               (-0.6664040438461246, -0.04429833352898575, 0.00502380522486473),
                               (1.1686968388986039, 0.8743086488169786, -0.4919298928897832),
                               (1.2813853343929593, -0.7114426553520238, 0.4734595111827543),
        ...
        atom_map = rxn.atom_map
        for index in [0, 3, 4]:
            self.assertIn(atom_map[index], [0, 1, 2])
        for index in [1, 2, 5, 6, 7, 8]:
            self.assertIn(atom_map[index], [3, 4, 5, 6, 7, 8])
        self.assertTrue(check_atom_map(rxn))
